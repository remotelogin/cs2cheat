...
void aimbot(pid_t game_pid, Display* aim_display) {
  for (unsigned long i = 0; i < 64; ++i) {
    ...
    uint64_t fFlag;
    Memory::read(game_pid, PlayerInfo::ptr_local_player + m_fFlag, &fFlag, sizeof(float[2])); 

    std::cout << fFlag << " fflag" << std::endl;

    const uint64_t STANDING = 65665;
    const uint64_t CROUCHING = 65667;
	...
	sleep(1);
	tmp = PLUS_JUMP;
	//	Memory::write(game_pid, Client::base_addr + dwForceJump, &tmp, sizeof(float[2]));
      }
    else
      {
	tmp = MINUS_JUMP;
